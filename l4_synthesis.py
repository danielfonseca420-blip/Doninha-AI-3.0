...
class RussellianSynthesisEngine:
    ...
    def synthesize(
        ...
        verified_response, verification_log = self.verifier.verify(
            ...
            context_summary=f"Hipótese principal: {best.proposition} | Estado L3: {best.state} | Certeza: {best.certainty:.2f}",
        )
    ...
    def _generate_response(self, best_pv: ParaconsistentValue, prompt: str, kb: Optional[Dict[str, float]] = None) -> str:
        ...
        source_note = (
            "Fontes bibliográficas/canonicais consultadas: Bertrand Russell, The Problems of Philosophy (cap. XII) + base local do projeto."
            if self.russell_base is not None
            else "Aviso de auditoria: nenhuma fonte bibliográfica verificável foi detectada na base local; a síntese L4 foi produzida com base semântica interna e sem citação externa confirmada."
        )
